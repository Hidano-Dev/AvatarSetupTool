...
- タイル境界のレンダリング誤差 (ラスタライズの浮動小数点差) — タイル矩形を出力ピクセル整数境界で定義し、カメラ矩形換算は double で計算して最後に float 化する。小サイズのタイル vs 単一描画一致テスト (剰余タイル・非正方形・非 2 冪分割を必須ケースに含む) で検証。実機で差が出た場合は AC 1.4 の許容 (各チャンネル ±1 階調以内) に収まることを確認して合格とし、単一パスフォールバックは採用しない
