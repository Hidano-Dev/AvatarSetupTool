...
- 推定出力ファイルサイズと出力時間の目安をウィンドウ内に常時表示 (メモリ見積もりと同じ欄。ProRes は特に大きくなりがちなため)。係数はテストモデルでの実測に基づき、サイズは実測の 1.3〜1.7 倍程度の安全率、時間は実測とほぼ一致 (環境により前後)
- MP4 の H.264 上限警告を 2 段階に拡張: 8192px など確実に超える設定ではエラー表示 + 撮影開始ボタンを無効化、4096px など横に広いモデルだけ超えうる設定では警告を表示。どちらも「ProRes 422 (MOV) に切り替える」ボタン付き

### Changed

- 10cm のグリッド細線を破線に変更 (実線の 1m 主線と区別しやすくするため)。破線の点は 2cm 周期 (点 1cm + 隙間 1cm) でワールド座標に揃い、線どうしの交点は必ず点の中心になる
...
- 10cm の細線を半透明 (α = 0.6) にし、Scene ビューのグリッドのように 1m の主線と視覚的に区別しやすくした

### Added

- MP4 (H.264) の解像度上限の事前チェックを追加。H.264 は規格上 Level 6 で 8K まで定義されているが、Unity の MediaEncoder が使う Windows のエンコーダは 4096x2304 相当までのため、超える設定では EditorWindow に警告と「ProRes 422 (MOV) に切り替える」ボタンを表示し、実行前チェックでも分かりやすいエラーを返す (従来は MediaEncoder の素の例外で失敗していた)
